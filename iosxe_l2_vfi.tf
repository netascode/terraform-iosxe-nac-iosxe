resource "iosxe_l2_vfi" "l2_vfi" {
  ...
  name   = try(local.device_config[each.value.name].l2_vfi.name, null)
  mode   = try(local.device_config[each.value.name].l2_vfi.mode, null)
  vpn_id = try(local.device_config[each.value.name].l2_vfi.vpn_id, null)
  neighbors = [for neighbor in try(local.device_config[each.value.name].l2_vfi.neighbors, []) : {
    ip_address    = neighbor.ip_address
    encapsulation = try(neighbor.encapsulation, null)
  }]
}
