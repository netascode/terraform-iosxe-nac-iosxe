  ...
  ikev2_profiles = flatten([
    for device in local.devices : [
      for profile in try(local.device_config[device.name].crypto.ikev2.profiles, []) : {
        ...
        match_address_local_interface_loopback = try(profile.match_address_local_interface_loopback, local.defaults.iosxe.configuration.crypto.ikev2.profiles.match_address_local_interface_loopback, null)
