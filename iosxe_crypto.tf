...
resource "iosxe_crypto_engine" "crypto_engine" {
  ...
  compliance_shield_disable = try(local.device_config[each.value.name].crypto.engine.compliance_shield_disable, local.defaults.iosxe.configuration.crypto.engine.compliance_shield_disable, null)
}
