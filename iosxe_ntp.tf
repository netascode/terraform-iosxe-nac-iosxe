resource "iosxe_ntp" "ntp" {
  ...
  trap_source_hundred_gig_e             = (try(local.device_config[each.value.name].ntp.trap_source, null) != null) && length([for e in try(local.device_config[each.value.name].interfaces.ethernets, []) : e.id if e.type == "HundredGigE" && e.id == try(local.device_config[each.value.name].ntp.trap_source, null)]) > 0 ? try(local.device_config[each.value.name].ntp.trap_source, null) : null
