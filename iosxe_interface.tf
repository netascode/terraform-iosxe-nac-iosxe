  ...
  interfaces_ethernets = flatten([
    for device in local.devices : [
      for int in try(local.device_config[device.name].interfaces.ethernets, []) : {
        ...
        switchport_trunk_native_vlan             = try(int.switchport.trunk_native_vlan_id, local.interfaces_ethernets_group_config[format("%s/%s", device.name, int.id)].switchport.trunk_native_vlan_id, local.defaults.iosxe.devices.configuration.interfaces.ethernets.switchport.trunk_native_vlan_id, null)
