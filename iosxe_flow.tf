  ...
  flow_exporter = flatten([
    for device in local.devices : [
      for exporter in try(local.device_config[device.name].flow.exporters, []) : {
        ...
        destination_ip                        = try(exporter.destination_ip, null)
        export_protocol                       = try(exporter.export_protocol, null)
        option_application_attributes_timeout = try(exporter.option_application_attributes_timeout, null)
        option_application_table_timeout      = try(exporter.option_application_table_timeout, null)
        option_interface_table_timeout        = try(exporter.option_interface_table_timeout, null)
        option_sampler_table                  = try(exporter.option_sampler_table, null)
        option_vrf_table_timeout              = try(exporter.option_vrf_table_timeout, null)
        source_loopback                       = try(exporter.source_loopback, null)
        source_gigabit_ethernet               = try(exporter.source_gigabit_ethernet, null)
        source_two_gigabit_ethernet           = try(exporter.source_two_gigabit_ethernet, null)
        source_five_gigabit_ethernet          = try(exporter.source_five_gigabit_ethernet, null)
        source_ten_gigabit_ethernet           = try(exporter.source_ten_gigabit_ethernet, null)
        source_twenty_five_gigabit_ethernet   = try(exporter.source_twenty_five_gigabit_ethernet, null)
        source_forty_gigabit_ethernet         = try(exporter.source_forty_gigabit_ethernet, null)
        source_hundred_gigabit_ethernet       = try(exporter.source_hundred_gigabit_ethernet, null)
        source_vlan                           = try(exporter.source_vlan, null)
        source_port_channel                   = try(exporter.source_port_channel, null)
        transport_udp                         = try(exporter.transport_udp, null)
        template_data_timeout                 = try(exporter.template_data_timeout, null)
        ttl                                   = try(exporter.ttl, null)
      }
    ]
  ])
}
  ...
  flow_monitor = flatten([
    for device in local.devices : [
      for monitor in try(local.device_config[device.name].flow.monitors, []) : {
        ...
        cache_timeout_active   = try(monitor.cache_timeout_active, null)
        cache_timeout_inactive = try(monitor.cache_timeout_inactive, null)
        record                 = try(monitor.record, null)
      }
    ]
  ])
}
  ...
  flow_record = flatten([
    for device in local.devices : [
      for record in try(local.device_config[device.name].flow.records, []) : {
        ...
        match_application_name                               = try(record.match.application_name, null)
        match_connection_client_ipv4_address                 = try(record.match.connection_client_ipv4_address, null)
        match_connection_client_ipv6_address                 = try(record.match.connection_client_ipv6_address, null)
        match_connection_server_ipv4_address                 = try(record.match.connection_server_ipv4_address, null)
        match_connection_server_ipv6_address                 = try(record.match.connection_server_ipv6_address, null)
        match_connection_server_transport_port               = try(record.match.connection_server_transport_port, null)
        match_datalink_vlan                                  = try(record.match.datalink_vlan, null)
        match_flow_direction                                 = try(record.match.flow_direction, null)
        match_flow_observation_point                         = try(record.match.flow_observation_point, null)
        match_interface_input                                = try(record.match.interface_input, null)
        match_ipv4_destination_address                       = try(record.match.ipv4_destination_address, null)
        match_ipv4_protocol                                  = try(record.match.ipv4_protocol, null)
        match_ipv4_source_address                            = try(record.match.ipv4_source_address, null)
        match_ipv4_tos                                       = try(record.match.ipv4_tos, null)
        match_ipv4_version                                   = try(record.match.ipv4_version, null)
        match_ipv6_destination_address                       = try(record.match.ipv6_destination_address, null)
        match_ipv6_protocol                                  = try(record.match.ipv6_protocol, null)
        match_ipv6_source_address                            = try(record.match.ipv6_source_address, null)
        match_ipv6_version                                   = try(record.match.ipv6_version, null)
        match_routing_vrf_input                              = try(record.match.routing_vrf_input, null)
        match_transport_destination_port                     = try(record.match.transport_destination_port, null)
        match_transport_source_port                          = try(record.match.transport_source_port, null)
        match_vxlan_vnid                                     = try(record.match.vxlan_vnid, null)
        match_vxlan_vtep_input                               = try(record.match.vxlan_vtep_input, null)
        match_vxlan_vtep_output                              = try(record.match.vxlan_vtep_output, null)
        match_datalink_mac_source_address_input              = try(record.match.datalink_mac_source_address_input, null)
        match_datalink_mac_destination_address_input         = try(record.match.datalink_mac_destination_address_input, null)
        match_datalink_source_vlan_id                        = try(record.match.datalink_source_vlan_id, null)
        match_datalink_destination_vlan_id                   = try(record.match.datalink_destination_vlan_id, null)
        match_ipv4_ttl                                       = try(record.match.ipv4_ttl, null)
        collect_connection_initiator                         = try(record.collect.connection_initiator, null)
        collect_connection_new_connections                   = try(record.collect.connection_new_connections, null)
        collect_connection_server_counter_bytes_network_long = try(record.collect.connection_server_counter_bytes_network_long, null)
        collect_connection_server_counter_packets_long       = try(record.collect.connection_server_counter_packets_long, null)
        collect_counter_bytes_long                           = try(record.collect.counter_bytes_long, null)
        collect_counter_packets_long                         = try(record.collect.counter_packets_long, null)
        collect_datalink_mac_source_address_input            = try(record.collect.datalink_mac_source_address_input, null)
        collect_flow_direction                               = try(record.collect.flow_direction, null)
        collect_interface_output                             = try(record.collect.interface_output, null)
        collect_timestamp_absolute_first                     = try(record.collect.timestamp_absolute_first, null)
        collect_timestamp_absolute_last                      = try(record.collect.timestamp_absolute_last, null)
        collect_transport_tcp_flags                          = try(record.collect.transport_tcp_flags, null)
      }
    ]
  ])
}
